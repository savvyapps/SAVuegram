	...
	export default {
		...
		methods: {
			...
			async signup() {
				this.performingRequest = true
				try{
				let user = await fb.auth.createUserWithEmailAndPassword(this.signupForm.email, this.signupForm.password)
				
						console.log(user.user, "fb.auth.createuser done")
						this.$store.commit('setCurrentUser', user.user)
						console.log(user.user.uid,"store.commit vuex")
						
						// create user obj
						let ref = fb.usersCollection.doc(user.user.uid)
						...
						console.log("fb.userconllection done.")
						this.$store.dispatch('fetchUserProfile')
						this.performingRequest = false
						this.$router.push('/dashboard')
				} catch(err) {
